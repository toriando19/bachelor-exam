...
// Add user interest (both in PostgreSQL and JSON)
router.get('/add-userinterest', async (req, res) => {
  try {
    ...
    // Add to PostgreSQL
    const resultPostgres = await addUserInterest(user_interest_user, user_interest_interest);

    // Add to JSON file
    await addUserInterestJSON(user_interest_user, user_interest_interest);

    res.json({ success: true, result: resultPostgres });
  } catch (error) {
...
// Remove user interest (both from PostgreSQL and JSON)
router.get('/remove-userinterest', async (req, res) => {
  try {
    ...
    const resultPostgres = await removeUserInterest(user_interest_user, user_interest_interest);

    // Remove from JSON file
    await removeUserInterestJSON(user_interest_user, user_interest_interest);
